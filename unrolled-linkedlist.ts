...
class BlockNode<T> {
  /**
   * `listLastNode` is `null` if the block is empty. Otherwise, it will always
   * point to the next element due to the circular nature of the linked list.
   */
  listLastNode: ListNode<T> | null;
  length: number;
  next: BlockNode<T>;
  constructor() {
    this.length = 0;
...
export class UnrolledLinkedlist<T> {
  private lastBlock: BlockNode<T> = new BlockNode<T>();
  private blockSize = 1;
  ...
  constructor(blockSize: number, arrayElement?: Array<T>) {
    ...
    if (arrayElement && arrayElement.length > blockSize * blockSize) {
      throw new Error(
        `Can't carry the whole array withing ${blockSize} block size`
      );
    }
    for (let i = 0; i < blockSize; i++) {
      ...
      for (; arrayElement && j < arrayElement.length && j < maxlistSize; j++) {
        const nListNode = new ListNode<T>(arrayElement[j]);
        this._appendListNodeToBlock(this.lastBlock, nListNode);
      }
      if (i > 0) this._appendOneBlock();
    }
  }
  ...
  private _appendListNodeToBlock(block: BlockNode<T>, node: ListNode<T>) {
    block.length++;
  ...
  private _distributeNodesToPreviousBlocks() {
    ...
    for (
      ;
      tBlock.next.length && tBlock != this.lastBlock;
      tBlock = tBlock.next
    ) {
      const nodesNeeded = newBlockSize - tBlock.length;
      if (nodesNeeded == 0) continue;
      const { head, lastNode, count } = this._breakNodesFromBlock(
        nodesNeeded,
        tBlock.next
      );
   ...
   */
  private _breakNodesFromBlock(count: number, block: BlockNode<T>) {
  ...
  private _getIndices(index: number) {
    return {
   ...
   */
  private _getBlockByIndex(index: number) {
   ...
   */
  private _shiftElementsBackward(block: BlockNode<T>) {
   ...
   */
  private _insertAt0(block: BlockNode<T>, node: ListNode<T>) {
    if (block.length == 0) {
      block.listLastNode = node;
      node.next = node;
    } else {
  ...
  private _shiftAElementForward(block: BlockNode<T>) {
   ...
   */
  loopOver(callbackFun: (data: T, index: number) => void) {
    let bHead = this.lastBlock.next;
    let index = 0;
